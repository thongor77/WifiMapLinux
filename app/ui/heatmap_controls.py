...
class HeatmapControls(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._check = QCheckBox("Heatmap")
        self._check.toggled.connect(self.toggled)
        layout.addWidget(self._check)

        self._sim_check = QCheckBox("Simulation")
        self._sim_check.setToolTip("Afficher la heatmap simulée (LDPL 3D) à partir des APs virtuels")
        self._sim_check.toggled.connect(self.sim_toggled)
        layout.addWidget(self._sim_check)

        layout.addWidget(QLabel("Réseau :"))
        ...
        self._section_btn = QPushButton("Coupe ↕")
        self._section_btn.setFixedHeight(24)
        self._section_btn.setToolTip("Tracer une ligne de coupe verticale")
    ...
    def populate_ssids(self, ssids: list[str]):
        ...
        self._ssid_combo.addItem("Meilleur signal", userData="")
